...
var function OnWeaponPrimaryAttack_titanweapon_laser_trip( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	if ( SoulHasPassive( soul, ePassives.PAS_ION_TRIPWIRE )  )			//ION equipment2
	{
		owner.AddSharedEnergy( 500 )
	}
	...
	function() : ( projectile, inflictor, tower, pylon, noSpawnIdx, team, pylonOrigin )
		{
			...
			if ( IsValid( soul ) )
			{
				...
				if ( IsValid( titan ) )
				{	
					if(SoulHasPassive( soul, ePassives.PAS_ION_TRIPWIRE)){			//Ion Equipment-2
						RadiusDamage(
							pylonOrigin,											// center
							titan,											// attacker
							inflictor,										// inflictor
							LASER_TRIP_DAMAGE,								// damage
							3*LASER_TRIP_DAMAGE_HEAVY_ARMOR,					// damageHeavyArmor
