...
var function OnWeaponPrimaryAttack_titanweapon_flightcore_rockets( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	if ( missile )
	{
		...
		if(SoulHasPassive(soul , ePassives.PAS_NORTHSTAR_FLIGHTCORE)){
			entity coretarget = expect entity(owner.s.vipertarget)
			if(coretarget != null)
				thread DelayedTrackingStartviper( missile, coretarget )
			else
...
void function DelayedTrackingStartviper( entity missile, entity target )
{
	missile.EndSignal( "OnDestroy" )
	wait 0.1
