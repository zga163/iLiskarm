...
float function HandleBlockingAndCalcDamageScaleForHit( entity blockingEnt, var damageInfo )
{
	if ( blockingEnt.IsTitan() )
	{
		...
		if ( blockingEnt.IsPlayer() && PlayerHasPassive( blockingEnt, ePassives.PAS_SHIFT_CORE ) && PlayerHasPassive( blockingEnt, ePassives.PAS_RONIN_SWORDCORE ) )
			return 0.1

		if ( blockingEnt.IsPlayer() && PlayerHasPassive( blockingEnt, ePassives.PAS_SHIFT_CORE ) )
			return SWORD_CORE_BLOCK_DAMAGE_REDUCTION
		
		if ( blockingEnt.IsPlayer() && PlayerHasPassive( blockingEnt, ePassives.PAS_RONIN_SWORDCORE ) )
			return 0.2
