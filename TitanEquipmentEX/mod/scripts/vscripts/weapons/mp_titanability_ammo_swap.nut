...
var function OnWeaponPrimaryAttack_ammo_swap( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	if ( weaponOwner.IsPlayer() ){
		...
		if ( IsValid( primaryWeapon ) && SoulHasPassive( soul, ePassives.PAS_LEGION_SMARTCORE ) )
		{
			primaryWeapon.SetWeaponPrimaryClipCount( minint(primaryWeapon.GetWeaponPrimaryClipCount()*2 ,primaryWeapon.GetWeaponPrimaryClipCountMax() ))
			#if SERVER
			if ( primaryWeapon.IsChargeWeapon() )
			primaryWeapon.SetWeaponChargeFractionForced( 0 )
