...
void function PAS_CooldownReduction_OnKill( entity victim, entity attacker, var damageInfo )
{
	...
	else
	{
		...
		if(IsValid(coreweapon)){
			if(coreweapon.GetWeaponClassName() == "mp_titancore_laser_cannon")
				attacker.AddSharedEnergy( 340 )
		}


		entity weaponleft = attacker.GetOffhandWeapon( OFFHAND_SPECIAL )

		switch ( GetWeaponInfoFileKeyField_Global( weaponleft.GetWeaponClassName(), "cooldown_type" ) )
		{
			case "grapple":
			...
			case "ammo_deployed":
			case "ammo_timed":
				int maxAmmo = weaponleft.GetWeaponPrimaryClipCountMax()
				weaponleft.SetWeaponPrimaryClipCountNoRegenReset( min( maxAmmo, weaponleft.GetWeaponPrimaryClipCount() + ( maxAmmo / 3 ) ) )
				break

			case "chargeFrac":
				weaponleft.SetWeaponChargeFraction( max( 0, weaponleft.GetWeaponChargeFraction() - 0.34 ) )
				break
	...
	//			break
			...
			default:
				Assert( false, weaponleft.GetWeaponClassName() + " needs to be updated to support cooldown_type setting" )
				break
		}

		entity weaponG = attacker.GetOffhandWeapon( OFFHAND_TITAN_CENTER )

		switch ( GetWeaponInfoFileKeyField_Global( weaponG.GetWeaponClassName(), "cooldown_type" ) )
		{
			case "grapple":
				attacker.SetSuitGrapplePower( attacker.GetSuitGrapplePower() + 34 )
				break
			...
			case "ammo_deployed":
			case "ammo_timed":
				int maxAmmo = weaponG.GetWeaponPrimaryClipCountMax()
				weaponG.SetWeaponPrimaryClipCountNoRegenReset( min( maxAmmo, weaponG.GetWeaponPrimaryClipCount() + ( maxAmmo / 3 ) ) )
				break

			case "chargeFrac":
				weaponG.SetWeaponChargeFraction( max( 0, weaponG.GetWeaponChargeFraction() - 0.34 ) )
				break
	...
	//			break
			...
			default:
				Assert( false, weaponG.GetWeaponClassName() + " needs to be updated to support cooldown_type setting" )
				break
		}

		entity weaponright = attacker.GetOffhandWeapon( OFFHAND_ORDNANCE )

		switch ( GetWeaponInfoFileKeyField_Global( weaponright.GetWeaponClassName(), "cooldown_type" ) )
		{
			case "grapple":
				attacker.SetSuitGrapplePower( attacker.GetSuitGrapplePower() + 34 )
				break
			...
			case "ammo_deployed":
			case "ammo_timed":
				int maxAmmo = weaponright.GetWeaponPrimaryClipCountMax()
				weaponright.SetWeaponPrimaryClipCountNoRegenReset( min( maxAmmo, weaponright.GetWeaponPrimaryClipCount() + ( maxAmmo / 3 ) ) )
				break

			case "chargeFrac":
				weaponright.SetWeaponChargeFraction( max( 0, weaponright.GetWeaponChargeFraction() - 0.34 ) )
				break
	...
	//			break
			...
			default:
				Assert( false, weaponright.GetWeaponClassName() + " needs to be updated to support cooldown_type setting" )
				break
		}
	}
}
...
void function WeaponAttackWave( entity ent, int projectileCount, entity inflictor, vector pos, vector dir, bool functionref( entity, int, entity, entity, vector, vector, int ) waveFunc )
{
	...
	{	
		...
		if("ispasflamecore" in ent.s){
			if(ent.s.ispasflamecore){
				maxCount = expect int( ent.ProjectileGetWeaponInfoFileKeyField( chargedPrefix + "wave_max_count" ) ) *300
				/*foreach(player in GetPlayerArray())
				{
					SendHudMessage(player, "right",-1,0.65,60,196,130,1,0,2,0)
				}*/
			}
	...
	else
	{
		...
		/*foreach(player in GetPlayerArray())
				{
					SendHudMessage(player, "damn",-1,0.65,60,196,130,1,0,2,0)
				}*/
	...
	for ( int i = 0; i < maxCount; i++ )
	{
		...
		if ( forwardTrace.fraction == 1.0 )
		{
			...
			if("isexfirecore" in ent.s){
				entity player = ent.GetOwner()
				
				thread swordline3fire(player , pos , pos + <0,0,2700> ,pos + <0,0,100>, 1)  
				CreateShake( pos + <0,0,20>, 33, 10 , 1 , 1100 )
			}
...
void function swordline3fire(entity player , vector stratpos , vector endpos,vector playerpos,int times)
{   
    if(!IsValid(player) )
        return
