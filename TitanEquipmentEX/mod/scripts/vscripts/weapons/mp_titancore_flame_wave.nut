...
global function CreateFlameWaveSegment
global function BeginScorchedEarth
...
var function OnWeaponPrimaryAttack_titancore_flame_wave( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	while ( count < offsets.len() )
	{
		...
		if(weapon.HasMod("exfire")){
			...
			entity projectile1 = weapon.FireWeaponGrenade( attackParams.pos + offset,<1.0,0,0.01>, < 0,0,0 >, FUSE_TIME, damageTypes.projectileImpact, damageTypes.explosive, shouldPredict, true, true )
			entity projectile2 = weapon.FireWeaponGrenade( attackParams.pos + offset, < -1.0,0,0.01>, < 0,0,0 >, FUSE_TIME, damageTypes.projectileImpact, damageTypes.explosive, shouldPredict, true, true )
			entity projectile3 = weapon.FireWeaponGrenade( attackParams.pos + offset, <0,1.0,0.01>, < 0,0,0 >, FUSE_TIME, damageTypes.projectileImpact, damageTypes.explosive, shouldPredict, true, true )
			entity projectile4 = weapon.FireWeaponGrenade( attackParams.pos + offset, <0,-1.0,0.01>, < 0,0,0 >, FUSE_TIME, damageTypes.projectileImpact, damageTypes.explosive, shouldPredict, true, true )
			entity projectile5 = weapon.FireWeaponGrenade( attackParams.pos + offset, <0.5,0.5,0.01>, < 0,0,0 >, FUSE_TIME, damageTypes.projectileImpact, damageTypes.explosive, shouldPredict, true, true )
			entity projectile6 = weapon.FireWeaponGrenade( attackParams.pos + offset, < -0.5,0.5,0.01>, < 0,0,0 >, FUSE_TIME, damageTypes.projectileImpact, damageTypes.explosive, shouldPredict, true, true )
			entity projectile7 = weapon.FireWeaponGrenade( attackParams.pos + offset, <0.5,-0.5,0.01>, < 0,0,0 >, FUSE_TIME, damageTypes.projectileImpact, damageTypes.explosive, shouldPredict, true, true )
			entity projectile8 = weapon.FireWeaponGrenade( attackParams.pos + offset, < -0.5,-0.5,0.01>, < 0,0,0 >, FUSE_TIME, damageTypes.projectileImpact, damageTypes.explosive, shouldPredict, true, true )
			if ( IsValid( projectile ) )
			{
				#if SERVER
					EmitSoundOnEntity( projectile, soundFXs[count] )
					weapon.EmitWeaponNpcSound( LOUD_WEAPON_AI_SOUND_RADIUS_MP, 0.5 )
					projectile.s.isexfirecore <- true
					projectile1.s.isexfirecore <- true
					projectile2.s.isexfirecore <- true
					projectile3.s.isexfirecore <- true
					projectile4.s.isexfirecore <- true
					projectile5.s.isexfirecore <- true
					projectile6.s.isexfirecore <- true
					projectile7.s.isexfirecore <- true
					projectile8.s.isexfirecore <- true
					if ( weapon.HasMod( "pas_scorch_flamecore" ) ){
						projectile.s.ispasflamecore <- true
						projectile1.s.ispasflamecore <- true
						projectile2.s.ispasflamecore <- true
						projectile3.s.ispasflamecore <- true
						projectile4.s.ispasflamecore <- true
						projectile5.s.ispasflamecore <- true
						projectile6.s.ispasflamecore <- true
						projectile7.s.ispasflamecore <- true
						projectile8.s.ispasflamecore <- true
					}

					thread BeginFlameWave( projectile, count, inflictor, attackParams.pos + offset, attackParams.dir )
					thread BeginFlameWave( projectile1, count+1, inflictora, attackParams.pos + offset, <1.0,0,0.01> )
					thread BeginFlameWave( projectile2, count+2, inflictorb, attackParams.pos + offset, < -1.0,0,0.01> )
					thread BeginFlameWave( projectile3, count+3, inflictorc, attackParams.pos + offset, <0,1.0,0.01> )
					thread BeginFlameWave( projectile4, count+4, inflictord, attackParams.pos + offset, <0,-1.0,0.01> )
					thread BeginFlameWave( projectile5, count+5, inflictore, attackParams.pos + offset, <0.5,0.5,0.01> )
					thread BeginFlameWave( projectile6, count+6, inflictorf, attackParams.pos + offset, < -0.5,0.5,0.01> )
					thread BeginFlameWave( projectile7, count+7, inflictorg, attackParams.pos + offset, <0.5,-0.5,0.01> )
					thread BeginFlameWave( projectile8, count+8, inflictorh, attackParams.pos + offset, < -0.5,-0.5,0.01> )
