...
void function OnWeaponStartZoomIn_titanweapon_sniper( entity weapon )
{
	...
	if ( weapon.HasMod( "pas_northstar_optics" ) )
	{
		...
		weaponOwner.s.isaiming <- true
		thread markenemy(weaponOwner)
	}
	#endif
}
...
void function markenemy(entity weaponOwner)
{

	while(true){
		if(!IsValid(weaponOwner) || !IsAlive(weaponOwner) || !weaponOwner.s.isaiming)
			break
		
		if(IsValid(weaponOwner) && IsAlive(weaponOwner)){
			vector attackPos = weaponOwner.GetOrigin()
			...
			entity target = TraceLine(traceStart,traceEnd,weaponOwner, TRACE_MASK_SHOT, TRACE_COLLISION_GROUP_NONE).hitEnt
			if(target != null){
				if(target.IsTitan()&&target.IsPlayer()&&target.GetTeam() != weaponOwner.GetTeam()){
					
					StatusEffect_AddTimed( target, eStatusEffect.sonar_detected, 1.0, 3, 3 )
				}
