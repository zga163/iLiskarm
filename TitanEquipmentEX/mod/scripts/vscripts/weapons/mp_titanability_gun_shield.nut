...
global const int PAS_LEGION_SHEILD_HEALTH = 500000
...
//	}
	...
	if(SoulHasPassive( soul, ePassives.PAS_LEGION_SPINUP ))
		health = 1000
	...
	if ( shieldWeapon.HasMod( "npc_infinite_shield" ) || SoulHasPassive( soul, ePassives.PAS_LEGION_GUNSHIELD ))
	{
		shieldWallFX.e.cpoint.SetOrigin( < 246.0, 20.0, 10.0 > ) // AMPED COLOR
	}
	else
	{	
...
void function CL_GunShield_Internal( entity player, entity vortexWeapon, entity vortexSphere )
{
	...
	while( true )
	{
		...
		if(!SoulHasPassive( player.GetTitanSoul(), ePassives.PAS_LEGION_GUNSHIELD ))
