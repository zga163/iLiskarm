...
void function OnProjectileCollision_titanability_sonar_pulse( entity projectile, vector pos, vector normal, entity hitEnt, int hitbox, bool isCritical )
{
		...
		bool hasDamageAmp = mods.contains( "pas_tone_sonar" )
		...
		if(hitEnt.IsPlayer() && hitEnt.IsTitan() && hitEnt.GetTeam() == owner.GetTeam()){
			if(IsValid(hitEnt.GetTitanSoul().soul.bubbleShield))
				return

			owner.s.isskill <- false
			cancelshield(owner)
			hitEnt.s.isskill <- true
			hitEnt.GetTitanSoul().nextRegenTime = Time()
			thread CreateParentedBubbleShield(hitEnt,hitEnt.GetOrigin(), normal ,5)

			entity Gweap = owner.GetOffhandWeapon( OFFHAND_TITAN_CENTER )      //G
                if( IsValid( Gweap ) )
                {
...
void function SonarPulseThink( entity enemy, vector position, int team, entity sonarOwner, bool hasIncreasedDuration, bool hasDamageAmp )
{
	...
	int statusEffect = 0
	if ( hasDamageAmp )
		statusEffect = StatusEffect_AddEndless( enemy, eStatusEffect.damage_received_multiplier, 0.4 )
