...
{	
	...
	if(SoulHasPassive(soul , ePassives.PAS_NORTHSTAR_CLUSTER)){
		entity missile = weapon.FireWeaponMissile( attackPos, attackDir, 0.55*missileSpeed, damageTypes.projectileImpact, damageTypes.explosive, doPopup, shouldPredict )
		thread EMPMissileThinkConstant(missile, player )
		entity effect = PlayFX(  $"EDP_base_2" , missile.GetOrigin() )
		effect.SetParent(missile)
		EffectSetControlPointVector( effect, 1 , <155,10,250> )
	}
	else{
