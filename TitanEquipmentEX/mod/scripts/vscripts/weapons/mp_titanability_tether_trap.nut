	...
	if ( shouldCreateProjectile )
	{
		...
		for ( int i = 0; i < numTethers; i++ )
		{
			...
			{	
				...
				{
					SetCustomSmartAmmoTarget( projectile, false ) // prevent friendly target lockon
					projectile.e.spawnTime = Time()
					//projectile.e.noOwnerFriendlyFire = false
					//projectile.SetInvulnerable()
					projectile.Hide()
					projectile.kv.VisibilityFlags = ENTITY_VISIBLE_TO_FRIENDLY
