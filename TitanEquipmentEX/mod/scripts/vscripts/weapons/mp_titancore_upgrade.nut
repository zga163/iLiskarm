	...
	entity soul = owner.GetTitanSoul()
		...
		if ( currentUpgradeCount == 0 )
		{
			if ( SoulHasPassive( soul, ePassives.PAS_VANGUARD_CORE1 ) )  // Arc Rounds
			{
				...
				if ( owner.IsPlayer() )
				{	
					Chat_ServerPrivateMessage( owner,  "\x1b[92mArc Rounds: \x1b[0m Increased Vortex Shield damage and magazine size,\x1b[94mACTIVATED \x1b[0m", false, true )
			...
			else if ( SoulHasPassive( soul, ePassives.PAS_VANGUARD_CORE2 ) ) //Missile Racks
			{
				...
				if ( owner.IsPlayer() )
				{	
					Chat_ServerPrivateMessage( owner, "\x1b[92mMissile Racks: \x1b[0m Increased rocket blast radius and fire rate,\x1b[94mACTIVATED  \x1b[0m", false, true )
					int conversationID = GetConversationIndex( "upgradeTo1" )
					Remote_CallFunction_Replay( owner, "ServerCallback_PlayTitanConversation", conversationID )
			...
			else if ( SoulHasPassive( soul, ePassives.PAS_VANGUARD_CORE3 ) ) //Energy Transfer
			{
				...
				if ( owner.IsPlayer() )
				{
					Chat_ServerPrivateMessage( owner, "\x1b[92mEnergy Transfer: \x1b[0m \x1b[94m[Energy Transfer]\x1b[0m grants allies core charge,\x1b[94mACTIVATED  \x1b[0m", false, true )
		...
		else if ( currentUpgradeCount == 1 )
		{
			if ( SoulHasPassive( soul, ePassives.PAS_VANGUARD_CORE4 ) )  // Rapid Rearm
			{
				...
				if ( owner.IsPlayer() )
				{
					Chat_ServerPrivateMessage( owner, "\x1b[92mRapid Rearm: \x1b[0m Faster reload and Rearm speed,\x1b[94mACTIVATED  \x1b[0m", false, true )					
			...
			else if ( SoulHasPassive( soul, ePassives.PAS_VANGUARD_CORE5 ) ) //Maelstrom
			{
				...
				if ( owner.IsPlayer() )
				{
					Chat_ServerPrivateMessage( owner, "\x1b[92mVortex Shield: \x1b[0m Grants shields to allies,\x1b[94mACTIVATED  \x1b[0m", false, true )		
					int conversationID = GetConversationIndex( "upgradeTo2" )
					Remote_CallFunction_Replay( owner, "ServerCallback_PlayTitanConversation", conversationID )
			...
			else if ( SoulHasPassive( soul, ePassives.PAS_VANGUARD_CORE6 ) ) //Energy Field
			{
				...
				if ( owner.IsPlayer() )
				{
					Chat_ServerPrivateMessage( owner, "\x1b[92mEnergy Field: \x1b[0m Expanded energy field radius,\x1b[94mACTIVATED  \x1b[0m", false, true )		
					int conversationID = GetConversationIndex( "upgradeTo2" )
					Remote_CallFunction_Replay( owner, "ServerCallback_PlayTitanConversation", conversationID )
		...
		else if ( currentUpgradeCount == 2 )
		{
			if ( SoulHasPassive( soul, ePassives.PAS_VANGUARD_CORE7 ) )  // Multi-Target Missiles
			{
				if ( owner.IsPlayer() )
				{	
					Chat_ServerPrivateMessage( owner, "\x1b[92mMulti-Target Missiles: \x1b[0m Increased damage and lock efficiency, bonus radius with \x1b[94m[Missile Racks]\x1b[0m,\x1b[94mACTIVATED  \x1b[0m", false, true )		
			...
			else if ( SoulHasPassive( soul, ePassives.PAS_VANGUARD_CORE8 ) ) //Superior Chassis
			{
				if ( owner.IsPlayer() )
				{
					Chat_ServerPrivateMessage( owner, "\x1b[92mSuperior Chassis: \x1b[0m Heals 1 bar on upgrade, full heal with \x1b[94m[Energy Transfer]\x1b[0m,\x1b[94mACTIVATED  \x1b[0m", false, true )		
					array<string> conversations = [ "upgradeTo3", "upgradeToFin" ]
					int conversationID = GetConversationIndex( conversations.getrandom() )
					Remote_CallFunction_Replay( owner, "ServerCallback_PlayTitanConversation", conversationID )
					...
					if ( !GetDoomedState( owner ) )
					{
						...
						//Hacky Hack - Append core_health_upgrade to setFileMods
						soul.soul.titanLoadout.setFileMods.append( "core_health_upgrade" )
						owner.SetHealth(min(owner.GetHealth()+2500 , owner.GetMaxHealth()))
			...
			{	
				if(owner.IsPlayer() && GetDoomedState( owner )){
					owner.s.eggcount <- 2
				}
				array<entity> weapons = GetPrimaryWeapons( owner )
				if ( weapons.len() > 0 )
				...
				if ( owner.IsPlayer() )
				{	
					Chat_ServerPrivateMessage( owner, "\x1b[92mXO-16 Accelerator: \x1b[0m Increased damage with stacking fire rate,\x1b[94mACTIVATED  \x1b[0m", false, true )
					array<string> conversations = [ "upgradeTo3", "upgradeToFin" ]
					int conversationID = GetConversationIndex( conversations.getrandom() )
					Remote_CallFunction_Replay( owner, "ServerCallback_PlayTitanConversation", conversationID )
...
void function rearmplayer(entity weaponOwner)
{
