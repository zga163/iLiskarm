...
void function OnWeaponDeactivate_titanweapon_heat_shield( entity weapon )
{
	EndVortex( weapon )
	entity weaponOwner = weapon.GetWeaponOwner()
	entity ind = expect entity(weaponOwner.s.shieldfx)
	if (IsValid(ind)){
		ind.Destroy()
	}
}
...
function StartHeatShield( entity weapon )
{
	...
	if ( weapon.GetWeaponChargeFraction() < 1 )
	{
		...
		if(!SoulHasPassive( soul , ePassives.PAS_SCORCH_SHIELD)){
			...
			CreateVortexSphere( weapon, false, false, 180 , 180)
			EnableVortexSphere( weapon )
			entity ind = CreateEntity( "prop_dynamic" )
			ind.SetValueForModelKey( $"models/fx/xo_emp_field.mdl" )
			ind.SetOrigin( weaponOwner.GetOrigin()+<0,0,150>)
			
			ind.kv.rendercolor = "81 130 151"
			DispatchSpawn( ind )
			ind.SetParent(weaponOwner)
			weaponOwner.s.shieldfx <- ind
		}
	}
	else
	{
	...
	while( true )
	{
		...
		if(!SoulHasPassive( soul , ePassives.PAS_SCORCH_SHIELD)){
			...
			RadiusDamage(
				...
				damageFarValue,									//damage,
				damageFarValueTitanArmor,						//damageHeavyArmor,
				explosionRadius*3,								//innerRadius,
				explosionRadius*6,								//outerRadius,
				SF_ENVEXPLOSION_NO_DAMAGEOWNER,					//flags,
				0.0,											// distanceFromAttacker
				0.0,											// explosionForce
				damageTypes.explosive,							//scriptDamageFlags,
